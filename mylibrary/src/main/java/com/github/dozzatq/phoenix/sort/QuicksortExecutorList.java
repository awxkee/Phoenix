...
class QuicksortExecutorList<T> {
    private List<T> sortArray;
    private int length;
    private Executor executor;
    private Quicksort.Comparator<T> comparator;
    private List<T> list;

    QuicksortExecutorList(Executor executor, List<T> sortArray, int length, Quicksort.Comparator<T> comparator) {
        if (executor == null)
            throw new NullPointerException("Executor must not be null!");
        if (sortArray == null)
            throw new NullPointerException("Sort list must not be null!");
        if (length < 0)
            throw new IllegalStateException("Length must be > 0!");

        this.sortArray = new ArrayList<>();
        this.sortArray.addAll(sortArray);
        this.length = length;
    ...
    Task<List<T>> sortAsync()
    {
        return Tasks.execute(executor, taskSource);
    }

    private TaskSource<List<T>> taskSource = new TaskSource<List<T>>() {
        @Override
    ...
    private void quicksort(int low, int high, @NonNull Quicksort.Comparator<T> comparator ) {
        ...
        T pivot = sortArray.get(low + (high-low)/2);

        // Divide into two lists
        while (i <= j) {
            // If the current value from the left list is smaller than the pivot
            // element then get the next element from the left list
            while (comparator.onCompare(sortArray.get(i), pivot) < 0) {
                i++;
            }
